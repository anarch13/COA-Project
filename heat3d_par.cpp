...
void heat3D_parallel(grid &heat) {
    grid newheat = heat;
    ...
    for (int t = 0; t < T; t++) {
        ...
        for (int i = 1; i < N-1; i++) {
            for (int j = 1; j < N-1; j++) {
                for (int k = 1; k < N-1; k++) {
                    #pragma omp await depend(i+1 j, k) depend(i-1, j, k) depend(i, j+1, k) depend(i, j-1, k) depend(i, j, k+1) depend(i, j, k-1)
